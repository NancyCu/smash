...
function WinnersContent() {
  ...
  // --- SMART ROLLOVER LOGIC (Matches GamePage) ---
  const rolloverResults = useMemo(() => {
      ...
      for (const q of quarters) {
          ...
          // Determine if quarter is "Finished"
          let isFinished = false;
          if (q === 'q1') isFinished = p > 1 || isHalf || isFinal;
          if (q === 'q2') isFinished = p > 2 || isFinal;
          ...
          // If quarter is DONE and NO WINNER -> Rollover happens
          if (isFinished && !hasWinner) {
              isRollover = true;
              finalPayout = 0; 
              carryOver = totalAmount; // Pass the buck
          } else if (hasWinner) {
              carryOver = 0; // Reset carryover
          } else {
              // Not finished yet -> Show potential payout
              carryOver = 0; // Don't carry forward yet, just show potential
          }
          ...
          results.push({
              ...
              label: labels[q],
              baseAmount,
              finalPayout: isRollover ? totalAmount : finalPayout, // Show amount that WAS rolled over
              isRollover,
              hasWinner,
              isFinished,
              ...result
    ...
    <div className="max-w-4xl mx-auto relative z-10 pb-20">
        ...
        <div className="grid gap-6">
            {rolloverResults.map((res, i) => {
                ...
                // Rollover style: dimmed background, red accent
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative z-10">
                            ...
                            <div className="flex flex-col items-center">
                                <div className={`p-3 rounded-full mb-2 ${
                                    ...
                                    : isRollover ? "bg-red-500/10 text-red-500 border border-red-500/20"
                                ...
                                {res.hasWinner ? (
                                    ...
                                    <div className="flex flex-col items-end">
                                        <div className={`px-4 py-2 rounded-xl border border-dashed text-sm italic mb-1 ${isRollover ? "bg-red-500/5 border-red-500/20 text-red-400" : "bg-white/5 border-white/10 text-slate-500"}`}>
                                            {isRollover ? "No Winner" : "Unclaimed Square"}
                                        </div>
                                    </div>
                                )}
                            </div>
