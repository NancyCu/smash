...
export default function GamePage() {
  ...
  // --- ROLLOVER CALCULATOR (Live Payment Schedule) ---
  const calculatedPayouts = useMemo(() => {
      ...
      // 1. Calculate Base Payouts (10/20/20/50)
      const base = {
      ...
      // 2. Helper to check if a square has a winner
      const hasWinner = (q: 'q1'|'q2'|'q3'|'final') => {
          if (!game.isScrambled || !game.axis) return true; // Assume winner if unscrambled
          ...
          return cell && (Array.isArray(cell) ? cell.length > 0 : true);
      };

      // 3. Determine if game is PAST that quarter (using ESPN data)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      let rollover = 0;
      let q1 = base.q1;
      // If Q1 is done (Period > 1 or Half or Final)
      if (period > 1 || isHalftime || isFinal) {
          if (!hasWinner('q1')) { rollover += q1; q1 = 0; }
      }

      // Q2 Check
      let q2 = base.q2 + rollover;
      rollover = 0; // Reset rollover holder
      if (period > 2 || (period === 2 && isFinal) || (isHalftime && isFinal)) { // Logic: if past Q2
           // Wait, simplicity: If Period > 2, Q2 is definitely done.
           if (period > 2 || isFinal) {
               if (!hasWinner('q2')) { rollover += q2; q2 = 0; }
           }
      } else {
          // If we are currently IN Q2 or haven't reached it, we just show the POTENTIAL payout (Base + Rollover)
      }

      // Q3 Check
      let q3 = base.q3 + rollover;
      rollover = 0;
      if (period > 3 || isFinal) {
          if (!hasWinner('q3')) { rollover += q3; q3 = 0; }
      }

      // Final Check
      const final = base.final + rollover;
  ...
  const infoPanel = (
    <GameInfo 
        ...
        // Pass the calculated LIVE payouts (with rollovers) instead of static config
