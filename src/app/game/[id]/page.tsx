...
export default function GamePage() {
  ...
  const currentScores = useMemo(() => {
    ...
    const defaultScores = { q1:{home:0,away:0}, q2:{home:0,away:0}, q3:{home:0,away:0}, final:{home:0,away:0}, teamA:0, teamB:0 };
    const savedScores = game?.scores || {};
    const mergedSavedScores = { ...defaultScores, ...savedScores };
  ...
  return (
    <main className="flex flex-col lg:flex-row h-screen w-full bg-[#0B0C15] overflow-hidden">
      ...
      <div className="lg:hidden p-4 bg-[#0B0C15]/90 sticky top-0 z-50 backdrop-blur-md flex justify-between items-center border-b border-white/5 shrink-0">
            ...
            <div className="flex bg-slate-800/50 p-1 rounded-lg border border-white/5 mx-4">
                <button 
      ...
      {/* 2. LEFT COLUMN: ACTION AREA (Grid + Score + Details) */}
      <div className={`flex-1 flex-col h-full relative overflow-y-auto lg:overflow-hidden no-scrollbar ${mobileView === 'board' ? 'flex' : 'hidden'} lg:flex`}>
            ...
            <div className="flex-1 flex flex-col items-center justify-start p-2 lg:p-6 w-full h-full lg:max-w-none lg:w-full gap-4">
            {/* A. JUMBOTRON SCOREBOARD */}
            <div className="w-full relative group z-20 shrink-0 max-w-3xl">
            ...
            {/* B. THE GRID */}
            <div className="w-full aspect-square max-w-[600px] flex items-center justify-center relative shrink-0">
            ...
            {/* C. SQUARE DETAILS (THE "ADD" BOX) - Only visible on Mobile below grid or if Desktop has space */}
            <div className="w-full max-w-[600px] bg-[#151725] border border-white/10 rounded-2xl p-4 shadow-xl lg:hidden">
      ...
      {/* 3. RIGHT SIDEBAR (Info + DESKTOP Details) */}
      <div className={`w-full lg:w-[420px] bg-[#0f111a] border-l border-white/5 overflow-y-auto z-20 shadow-2xl h-full lg:h-full ${mobileView === 'info' ? 'flex flex-col flex-1' : 'hidden'} lg:flex flex-col`}>
         ...
         <div className="p-6 space-y-8">
            ...
            {/* DESKTOP SQUARE DETAILS (Moved here for better UX) */}
            <div className="hidden lg:block bg-[#151725] border border-white/10 rounded-2xl p-4 shadow-xl">
               <div className="flex items-center justify-between mb-4 border-b border-white/5 pb-2">
                  <div className="flex flex-col">
                      <span className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">Selected Square</span>
                      <span className="text-lg font-black text-white">
                        {selectedCell ? `Row ${currentAxis.row[selectedCell.row]} • Col ${currentAxis.col[selectedCell.col]}` : "None Selected"}
                      </span>
                  </div>
                  {selectedCell && (
                    <button onClick={() => setSelectedCell(null)} className="p-1 rounded-full hover:bg-white/10 text-slate-500 transition-colors">
