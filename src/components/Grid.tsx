...
export default function Grid({ 
  ...
  return (
    <div className="relative grid grid-cols-11 border border-white/5 bg-[#0f111a] select-none">
      ...
      {!isScrambled && (
        <>
          {/* Top Header Overlay (Team B) */}
          <div className="absolute top-0 left-[9.09%] right-0 h-8 md:h-10 z-10 flex items-center justify-center pointer-events-none">
             <span className="font-black text-xl md:text-2xl uppercase tracking-widest text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
                {teamB}
             </span>
          </div>

          {/* Left Header Overlay (Team A) */}
          <div className="absolute top-[32px] md:top-[40px] bottom-0 left-0 w-8 md:w-10 z-10 flex items-center justify-center pointer-events-none">
             <span className="font-black text-xl md:text-2xl uppercase tracking-widest text-white -rotate-90 whitespace-nowrap drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
                {teamA}
             </span>
          </div>
        </>
      ...
      <div className="contents">
         ...
         {cols.map((num, i) => (
           <div key={`col-${i}`} className={`relative p-1 h-8 md:h-10 flex items-center justify-center border-b border-r border-white/5 bg-[#151725] ${winningCell?.col === i ? 'bg-cyan-900/30' : ''}`}>
             ...
             {/* State A: Scrambled (Show Numbers) */}
             {isScrambled ? (
                <>
             ...
             /* State B: Unscrambled (Show Faded Logo) */
                teamBLogo && (
                  <img src={teamBLogo} alt="" className="h-full w-full object-contain opacity-20 grayscale invert md:p-1" />
                )
             )}
           </div>
         ))}
      ...
      {rows.map((rowNum, rIndex) => (
        <div key={`row-${rIndex}`} className="contents">
            ...
            <div className={`relative w-8 md:w-10 flex items-center justify-center border-r border-b border-white/5 bg-[#151725] ${winningCell?.row === rIndex ? 'bg-pink-900/30' : ''}`}>
               ...
               /* State B: Unscrambled (Show Faded Logo) */
                  teamALogo && (
                    <img src={teamALogo} alt="" className="h-full w-full object-contain opacity-20 grayscale invert md:p-1" />
                  )
               )}
            </div>
